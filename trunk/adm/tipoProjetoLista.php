<?php
	$path_root_tipoProjetoLista = dirname(__FILE__);
	$DS = DIRECTORY_SEPARATOR;
	$path_root_tipoProjetoLista = "{$path_root_tipoProjetoLista}{$DS}..{$DS}";
	include_once "{$path_root_tipoProjetoLista}adm{$DS}includes{$DS}header.php";
	include_once("{$path_root_tipoProjetoLista}adm{$DS}class{$DS}tipoProjeto.class.php");
	$obj = new tipoProjeto();
...
<div id="contentWrapper">
	<div id="breadCrumbs">
          Painel Administrativo <strong>/ Tipos de Projeto</strong>
	</div>

	<div class="left" style="width:auto;">
		<form action="tipoProjetoLista.php" method="post" id="formBusca" name="formbusca">
	...
	<div class="right" style="float:right;width:auto;">
		<a href="tipoProjetoEdicao.php" class="butCadastro">Cadastrar novo Tipo de Projeto</a>
	</div>
	<br clear="all" />
	<table cellpadding="8" cellspacing="0" border="0" width="100%">
		...
		<tbody>
			...
			<tr class="tableItem<?=$evenOdd?>">
				<td align="center"><?=$v['tipo_projeto_id']?></td>
				<td><?=$v['tipo_projeto_titulo']?></td>
				<td>
					<a href="tipoProjetoEdicao.php?tipo_projeto_id=<?=$v['tipo_projeto_id']?>" class="btEdit">Editar</a>
					<a href="javascript:void(0);" rel="<?=$v['tipo_projeto_id']?>" class="btDel">Excluir</a>
				</td>
			</tr>
			<?		endforeach;?>
			<?	else:?>
...
<?php include_once "{$path_root_tipoProjetoLista}adm{$DS}includes{$DS}footer.php"; ?>
