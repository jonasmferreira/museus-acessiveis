<?php
	$path_root_tagLista = dirname(__FILE__);
	$DS = DIRECTORY_SEPARATOR;
	$path_root_tagLista = "{$path_root_tagLista}{$DS}..{$DS}";
	include_once "{$path_root_tagLista}adm{$DS}includes{$DS}header.php";
	include_once("{$path_root_tagLista}adm{$DS}class{$DS}tag.class.php");
	$obj = new tag();
...
<div id="contentWrapper">
	<div id="breadCrumbs">
          Painel Administrativo <strong>/ Tags</strong>
	</div>

	<div class="left" style="width:auto;">
		<form action="tagLista.php" method="post" id="formBusca" name="formbusca">
	...
	<div class="right" style="float:right;width:auto;">
		<a href="tagEdicao.php" class="butCadastro">Cadastrar novo tag</a>
	</div>
	<br clear="all" />
	<table cellpadding="8" cellspacing="0" border="0" width="100%">
		...
		<tbody>
			...
			<tr class="tableItem<?=$evenOdd?>">
				...
				<td><?=$v['tag_titulo']?></td>
				<td>
					<a href="tagEdicao.php?tag_id=<?=$v['tag_id']?>" class="btEdit">Editar</a>
					<a href="javascript:void(0);" rel="<?=$v['tag_id']?>" class="btDel">Excluir</a>
				</td>
			</tr>
			<?		endforeach;?>
			<?	else:?>
...
<?php include_once "{$path_root_tagLista}adm{$DS}includes{$DS}footer.php"; ?>
