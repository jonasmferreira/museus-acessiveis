<?php
	$path_root_quemSomosLista = dirname(__FILE__);
	$DS = DIRECTORY_SEPARATOR;
	$path_root_quemSomosLista = "{$path_root_quemSomosLista}{$DS}..{$DS}";
	include_once "{$path_root_quemSomosLista}adm{$DS}includes{$DS}header.php";
	include_once("{$path_root_quemSomosLista}adm{$DS}class{$DS}quemSomos.class.php");
	$obj = new quemSomos();
...
<div id="contentWrapper">
	<div id="breadCrumbs">
          Painel Administrativo <strong>/ Quem Somos</strong>
	</div>
	...
	<div class="right" style="float:right;width:auto;">
		<a href="quemSomosEdicao.php" class="butCadastro">Adicionar</a>
	</div>
	<br clear="all" />
	<table cellpadding="8" cellspacing="0" border="0" width="100%">
		...
		<tbody>
			...
			<tr class="tableItem<?=$evenOdd?>">
				<td align="center"><?=$v['quemsomos_id']?></td>
				<td>
					<?php 
						$aDt = explode(' ', $v['quemsomos_dt_cadastro']);
						$sDt = $aDt[0];
						echo $sDt;
					?>
				</td>
				<td><span <?php echo ($v['quemsomos_exibir']=='N') ?'style="color:#FF0000 !important;"' :'';?>><?=$v['quemsomos_exibir_label']?></span></td>
				<td><?=$v['quemsomos_titulo']?></td>
				<td>
					<a href="quemSomosEdicao.php?quemsomos_id=<?=$v['quemsomos_id']?>" class="btEdit">Editar</a>
					<a href="javascript:void(0);" rel="<?=$v['quemsomos_id']?>" class="btDel">Excluir</a>
				</td>
			</tr>
			<?		endforeach;?>
			<?	else:?>
...
<?php include_once "{$path_root_quemSomosLista}adm{$DS}includes{$DS}footer.php"; ?>
