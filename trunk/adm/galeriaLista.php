<?php
	$path_root_galeriaLista = dirname(__FILE__);
	$DS = DIRECTORY_SEPARATOR;
	$path_root_galeriaLista = "{$path_root_galeriaLista}{$DS}..{$DS}";
	include_once "{$path_root_galeriaLista}adm{$DS}includes{$DS}header.php";
	include_once("{$path_root_galeriaLista}adm{$DS}class{$DS}galeria.class.php");
	$obj = new galeria();
...
<div id="contentWrapper">
	<div id="breadCrumbs">
          Painel Administrativo <strong>/ Galeria</strong>
	</div>

	<div class="left" style="width:auto;">
		<form action="galeriaLista.php" method="post" id="formBusca" name="formbusca">
	...
	<div class="right" style="float:right;width:auto;">
		<a href="galeriaEdicao.php" class="butCadastro">Adicionar</a>
	</div>
	<br clear="all" />
	<table cellpadding="8" cellspacing="0" border="0" width="100%">
		...
		<tbody>
			...
			<tr class="tableItem<?=$evenOdd?>">
				<td align="center"><?=$v['galeria_id']?></td>
				<td align="left"><?=$v['galeria_titulo']?></td>
				<td>
					<a href="galeriaEdicao.php?galeria_id=<?=$v['galeria_id']?>" class="btEdit">Editar</a>
					<a href="javascript:void(0);" rel="<?=$v['galeria_id']?>" class="btDel">Excluir</a>
				</td>
			</tr>
			<?		endforeach;?>
			<?	else:?>
...
<?php include_once "{$path_root_galeriaLista}adm{$DS}includes{$DS}footer.php"; ?>
