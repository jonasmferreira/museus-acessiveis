<?php
$path_root_clippingController = dirname(__FILE__);
$DS = DIRECTORY_SEPARATOR;
$path_root_clippingController = "{$path_root_clippingController}{$DS}..{$DS}..{$DS}";
require_once "{$path_root_clippingController}adm{$DS}class{$DS}clipping.class.php";
$obj = new clipping();
switch($_REQUEST['action']){
	case 'edit-item':
		$volta = "clippingEdicao.php";
		...
		$obj->setFiles($_FILES);
		$exec = $obj->edit();
		if(isset($_POST['clipping_id']) && trim($_POST['clipping_id'])!=''){
			if($exec['success']){
				$msg = "Dados atualizados com sucesso!";
				$url = "{$volta}?clipping_id={$_POST['clipping_id']}";
			}else{
	...
	case 'down-order':
		...
		$aResult = $obj->getDownloadByClipping($downId,$sOrder,'ASC');
		if(count($aResult)>0){
			$sTable="";
