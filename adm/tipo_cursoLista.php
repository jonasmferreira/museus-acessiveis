<?php
	$path_root_tipo_cursoLista = dirname(__FILE__);
	$DS = DIRECTORY_SEPARATOR;
	$path_root_tipo_cursoLista = "{$path_root_tipo_cursoLista}{$DS}..{$DS}";
	include_once "{$path_root_tipo_cursoLista}adm{$DS}includes{$DS}header.php";
	include_once("{$path_root_tipo_cursoLista}adm{$DS}class{$DS}tipo_curso.class.php");
	$obj = new tipo_curso();
...
<div id="contentWrapper">
	<div id="breadCrumbs">
          Painel Administrativo <strong>/ Tipo de Cursos</strong>
	</div>

	<div class="left" style="width:auto;">
		<form action="tipo_cursoLista.php" method="post" id="formBusca" name="formbusca">
	...
	<div class="right" style="float:right;width:auto;">
		<a href="tipo_cursoEdicao.php" class="butCadastro">Cadastrar novo tipo_curso</a>
	</div>
	<br clear="all" />
	<table cellpadding="8" cellspacing="0" border="0" width="100%">
		...
		<tbody>
			...
			<tr class="tableItem<?=$evenOdd?>">
				<td align="center"><?=$v['tipo_curso_id']?></td>
				<td><?=$v['tipo_curso_titulo']?></td>
				<td>
					<a href="tipo_cursoEdicao.php?tipo_curso_id=<?=$v['tipo_curso_id']?>" class="btEdit">Editar</a>
					<a href="javascript:void(0);" rel="<?=$v['tipo_curso_id']?>" class="btDel">Excluir</a>
				</td>
			</tr>
			<?		endforeach;?>
			<?	else:?>
...
<?php include_once "{$path_root_tipo_cursoLista}adm{$DS}includes{$DS}footer.php"; ?>
