		...
		foreach( $files as $file ) {
			if( file_exists($root . $_POST['dir'] . $file) && $file != '.' && $file != '..' && !is_dir($root . $_POST['dir'] . $file) ) {
				...
				if(in_array(strtolower($ext),array('jpg','jpeg','gif','png','bmp'))){
					$arq = htmlentities($_POST['dir'] . $file);
					$arq = str_replace("../../../","../",$arq);
					$thumb = "<img src='{$arq}' style='width:60px;height:60px' /><br />";
					echo "
						<li class=\"file\" style='background:none'>
							<a href=\"#\" rel=\"" . htmlentities($_POST['dir'] . $file) . "\">{$thumb}" . htmlentities($file) . "</a>
						</li>
					";
				}else{
					echo "
